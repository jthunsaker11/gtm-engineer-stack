...
8. **Delegation pipeline (scoring, contacts, email)**. After sourcing and signal enrichment, run each account through the delegated skills in order:
   a. Call **icp-scoring** with the account, its enriched signals, and `fresh_signals`: the signals that survived the freshness gate below, each with its canonical type from the signal type vocabulary in config/offering.md. Pass the types, not just a count, because icp-scoring weights them. It returns `tier` (A/B/C, or `skip`), the fit `score`, `fit_band`, `fresh_signal_weight`, `tier_cap_reason`, and `recommended_persona`. The tier is the worse of the fit band and the fresh-signal cap, so a high-fit account with no live signal comes back Tier C with its fit score intact; persist both (`ab_fit_score`, `ab_tier_cap_reason`) so the row reads as "good account, wrong moment" rather than as a downgrade.
...
| Employee-size band | `organization_num_employees_ranges`, taken verbatim from the preflight gate's `PREFLIGHT_APOLLO_BUCKETS` | Never hand-typed. `hooks/preflight-config.sh` derives the buckets from the employee range in config/icp.md and emits them; the run uses that value and no other. Apollo's bands are fixed, so when the gate reports `PREFLIGHT_POST_SOURCE_REFILTER=required` the query is wider than the ICP and the pool must be re-filtered post-source on `PREFLIGHT_EMP_MIN` to `PREFLIGHT_EMP_MAX`, with the dropped-row count reported. |
...
4. **Delegate to icp-scoring** (per account): hand it the account, its signals, and the post-gate `fresh_signals` with their canonical types; record the `tier`, the fit `score`, `fresh_signal_weight`, `tier_cap_reason`, and `recommended_persona` it returns.
...
- `deduped_open_sales_roles` (the numerator) is the deduped, entity-matched open-sales-role count from `Company Job Openings` (VP Sales, AE, SDR, Sales Manager), per the Company Job Openings hardening below, so duplicate postings cannot inflate it. Persist it as `ab_signal_sales_roles`; a ratio whose numerator is not recorded cannot be audited. It carries the `hiring_signal` freshness window defined in config/offering.md, so stale postings do not fire the signal.
...
- **Default: CSV.** Write the aggregated audience to a local CSV with columns for firmographics, the source (`ab_pool_source`), each signal (including `ab_funding_round`, `ab_signal_leadership_hire`, `ab_signal_sales_team_growth`, `ab_sales_team_growth_ratio`, `ab_sales_team_growth_fires`, `ab_sales_team_growth_reason`, `ab_signal_sales_roles`, `ab_signal_hiring_raw`, and `ab_signal_hiring_warning`), the tier from icp-scoring plus its fit score and cap reason (`ab_tier`, `ab_fit_score`, `ab_tier_cap_reason`), the intended motion (`ab_motion`), the resolved contact (tier A/B rows), the email source (`ab_email_source`), the email-verification verdict, and the provenance and audit columns (`ab_pool`, `ab_source`, `ab_enriched_at`, `ab_expired_signals`). Tell the user how to import it in the Clay app (New table, then CSV import) if they want it in Clay.
...
- `ab_signal_funding` (the funding signal: the round amount from Company Latest Funding, plus the round date from Apollo `latest_funding_round_date`. The date is required rather than optional, because the freshness gate expires any signal it cannot date, so a funding signal without one never scores.)
- `ab_signal_sales_roles` (the deduped, entity-matched open sales-role count: VP Sales, AE, SDR, Sales Manager, drawn from the same deduped set as `ab_signal_hiring`. This is the numerator of `ab_sales_team_growth_ratio` and is persisted in its own right, because a ratio whose numerator is not recorded cannot be audited.)
