...
- Deal size: ACV $20K to $200K (human judgment). No available field carries a prospect's ACV, because it is a property of the deal rather than of the account. For a seat-based product it follows from the sales-team size already gated above, so it is a sanity check a person applies when reviewing the pool, not an independent filter.
